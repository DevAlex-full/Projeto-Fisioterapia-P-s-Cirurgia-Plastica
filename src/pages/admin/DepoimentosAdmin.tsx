    ...
    <AdminLayout title="Depoimentos">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center overflow-y-auto">
          <div className="bg-white w-full sm:rounded-3xl sm:max-w-xl sm:my-6 min-h-screen sm:min-h-0">
            <div className="sticky top-0 z-10 bg-white flex items-center justify-between px-5 py-4 border-b border-[#F5F1EB] sm:rounded-t-3xl">
              <h3 className="text-lg font-bold text-[#5D4E37]">{editing ? 'Editar' : 'Novo'} Depoimento</h3>
            ...
            <div className="p-5 sm:p-8 space-y-4">
              ...
              <div className="bg-[#F5F1EB] rounded-2xl p-4 flex gap-3 items-start">
                {form.fotoUrl
                  ...
                  : <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#D4AF7A] to-[#8B7355] flex-shrink-0 flex items-center justify-center text-white font-bold text-lg">
                      {form.nome?.[0] || '?'}
                    </div>}
                <div className="min-w-0">
                  <p className="font-bold text-[#5D4E37] text-sm truncate">{form.nome || 'Nome do paciente'}</p>
                  <p className="text-xs text-[#D4AF7A]">{form.procedimento || 'Procedimento'}</p>
                  <div className="flex gap-0.5 my-1">
                    {[1,2,3,4,5].map(n => <Star key={n} className={`w-3 h-3 ${n <= form.rating ? 'text-[#D4AF7A] fill-current' : 'text-[#ddd]'}`} />)}
                  </div>
              ...
              {[{ key: 'nome', label: 'Nome do Paciente' }, { key: 'procedimento', label: 'Procedimento Realizado' }].map(f => (
                <div key={f.key}>
                  ...
                  <div className="flex gap-1">
                    {[1,2,3,4,5].map(n => (
                      <button key={n} type="button" onClick={() => setForm({ ...form, rating: n })}>
                        <Star className={`w-7 h-7 transition ${n <= form.rating ? 'text-[#D4AF7A] fill-current' : 'text-[#ddd]'}`} />
                      </button>
                    ))}
                  </div>
                </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3 sm:space-y-4">
          {items.map(d => (
            <div key={d.id} className={`bg-white rounded-2xl p-4 sm:p-5 shadow-md border-2 flex gap-3 sm:gap-4 ${d.ativo ? 'border-[#F5F1EB]' : 'border-red-100 opacity-60'}`}>
              {d.fotoUrl
                ? <img src={d.fotoUrl} alt={d.nome} className="w-12 h-12 sm:w-14 sm:h-14 rounded-full object-cover flex-shrink-0" />
                : <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br from-[#D4AF7A] to-[#8B7355] flex-shrink-0 flex items-center justify-center text-white font-bold text-lg">{d.nome?.[0]}</div>}
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-bold text-[#5D4E37] text-sm sm:text-base truncate">{d.nome}</p>
                    <p className="text-xs text-[#D4AF7A] font-medium">{d.procedimento}</p>
                    <div className="flex gap-0.5 my-1">
                      {[...Array(d.rating)].map((_, i) => <Star key={i} className="w-3 h-3 text-[#D4AF7A] fill-current" />)}
                    </div>
                  </div>
                  <div className="flex gap-1 flex-shrink-0">
                    <button onClick={() => toggleAtivo(d)} className="p-1.5 hover:bg-[#F5F1EB] rounded-lg">
                      {d.ativo ? <ToggleRight className="w-5 h-5 text-green-500" /> : <ToggleLeft className="w-5 h-5 text-[#999]" />}
                    </button>
                ...
                <p className="text-xs sm:text-sm text-[#666] line-clamp-2 mt-1">"{d.texto}"</p>
              </div>
            </div>
          ))}
        </div>
