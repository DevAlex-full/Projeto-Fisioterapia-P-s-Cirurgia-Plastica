...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    catch { setError('Email ou senha inválidos. Tente novamente.'); }
    finally { setLoading(false); }
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F5F1EB] via-white to-[#F5F1EB]
      flex items-center justify-center p-4">
      <div className="w-full max-w-sm sm:max-w-md">

        {/* Logo */}
        <div className="text-center mb-8">
          <div className="w-24 h-24 rounded-3xl overflow-hidden mx-auto mb-4 shadow-2xl">
        ...
        {/* Card */}
        <div className="bg-white rounded-3xl shadow-2xl p-6 sm:p-8 border-2 border-[#F5F1EB]">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-[#F5F1EB] border-2 border-transparent
                    rounded-2xl focus:outline-none focus:border-[#D4AF7A] focus:bg-white transition text-[#333]" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPassword ? 'text' : 'password'} value={password}
                  ...
                  className="w-full pl-12 pr-12 py-4 bg-[#F5F1EB] border-2 border-transparent
                    rounded-2xl focus:outline-none focus:border-[#D4AF7A] focus:bg-white transition text-[#333]" />
                <button type="button" onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-[#999] hover:text-[#D4AF7A] transition">
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-gradient-to-r from-[#D4AF7A] to-[#8B7355] text-white py-4
                rounded-2xl font-bold hover:shadow-xl transition disabled:opacity-60
                flex items-center justify-center gap-2">
              {loading
                ? <><Loader className="w-5 h-5 animate-spin" /> Entrando...</>
                : 'Entrar'}
            </button>
          </form>
        </div>
